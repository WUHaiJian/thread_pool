...
static struct thread* _thread_get_th(struct threadpool* pool, pthread_t tid)
{
    struct thread* head = pool->threads;
...
static pthread_cond_t* _thread_get_idle(struct threadpool* pool)
{
    struct thread* head = pool->threads;
...
static int _thread_get_cancel(struct threadpool* pool, pthread_t tid)
{
    struct thread* head = pool->threads;
...
static void* _consumer(struct threadpool* pool)
{
    ...
    // 改消费者线程加锁, 并且得到当前线程的条件变量,vdel_f为了linux上消除警告
    pthread_cleanup_push((vdel_f)pthread_mutex_unlock, mtx);
    pthread_mutex_lock(mtx);

    cnd = _thread_get(pool, tid);
	struct thread * th = _thread_get_th(pool, tid);
__loop:
	if (_thread_get_cancel(pool, tid))
		goto __CANCEL;
...
sp_add(threadpool_t pool, vdel_f run, void* arg)
{
    ...
    if(pool->idle > 0){
        ...
        pthread_cond_t * cond = _thread_get_idle(pool);
		pthread_mutex_unlock(mtx);
